...
typedef struct	s_img
{
	void	*img_ptr;
	int		*data;
	//아래 3개 값은 이해 안해도 사용하는데 지장 없다.
	//선언한 뒤 함수의 인자로만 잘 넣어주면 알아서 정보를 받아나옴.
	int		size_l;
	int		bpp;
	int		endian;
}				t_img;
...
int				main(void)
{
	...
	//여기서 (int *)를 안해주고 img.data가 void *이면 밑에 배열값들을 참조할 때 다 4를 곱해야 한다.
	//그렇기 때무넹 int *로 캐스팅해주는 편이 좋다고 한다.)
	count_h = -1;
